...
@Component
public class MongoApplyOnyxdbAgentServiceTaskConsumer extends ClusterTaskConsumer {
    private final KubernetesAdapter kubernetesAdapter;
    private final ProjectRepository projectRepository;

    public MongoApplyOnyxdbAgentServiceTaskConsumer(
    ...
    public TaskType getTaskType() {
        return TaskType.MONGO_APPLY_ONYXDB_AGENT_SERVICE;
    }
    ...
    protected TaskResult internalProcess(Task task, ClusterPayload payload) {
        ...
        kubernetesAdapter.applyOnyxdbAgentService(
                project.namespace(),
                project.name(),
                cluster.name()
        );
